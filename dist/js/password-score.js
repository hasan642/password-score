...
import options from "./password-score-options";
    ...
    this.areAdjacent = function(a, b) {
        if (a in this.keyboard) {
            for (var i = 0; i < this.keyboard[a].length; i++) {
                if (
                    this.keyboard[a][i] !== null &&
                    this.keyboard[a][i].indexOf(b) >= 0
                ) {
                    return true;
                }
            }
...
Score.prototype = {
            ...
            switch (options[i]["type"]) {
                ...
                case "dictionary":
                    if ("dictionary" in options[i]) {
                        optionMatches = this.collectDictionaryMatches(
                            options[i]["dictionary"]
                        );

                        if ("leet" in options[i] && options[i]["leet"] === true) {
                            var leetMatches = this.collectLeetSpeakMatches(
                                options[i]["dictionary"]
                            );
            ...
            for (var j = i; j < this.password.length; j++) {
                ...
                if (string in dictionary) {
                    if (dictionary[string]) {
                        matches[matches.length] = {
                            pattern: original,
                            entropy: this.calculateDictionaryEntropy(
                                original,
                                string,
                                dictionary[string]
                            ),
                ...
                if (reversed in dictionary) {
                    if (dictionary[reversed]) {
                        matches[matches.length] = {
                            pattern: original,
                            entropy: this.calculateReversedDictionaryEntropy(
                                original,
                                string,
                                dictionary[reversed]
                            ),
    ...
    calculateDictionaryEntropy: function(original, word, rank) {
        if (
            this.regex["lower"].test(original) &&
            this.regex["upper"].test(original)
        ) {
                ...
                for (var j = i; j < subs[k].length; j++) {
                    ...
                    if (string in dictionary) {
                        if (dictionary[string]) {
                            matches[matches.length] = {
                                ...
                                entropy: this.calculateLeetSpeakEntropy(
                                    this.password.substring(i, j + 1),
                                    string,
                                    dictionary[string]
                                ),
                                start: i,
                    ...
                    if (reversed in dictionary) {
                        if (dictionary[string]) {
                            matches[matches.length] = {
                                pattern: originalPattern,
                                entropy: this.calculateReversedLeetSpeakEntropy(
                                    this.password.substring(i, j + 1),
                                    string,
                                    dictionary[string]
                                ),
    ...
    calculateLeetSpeakEntropy: function(original, word, rank) {
        ...
        return (
            this.calculateDictionaryEntropy(original, word, rank) +
            this.lg(possibilities)
        );
    },
        ...
        var recursiveSubstitutions = function(string) {
            if (string[0] in leet) {
                if (string.length === 1) {
                    ...
                    var substrings = recursiveSubstitutions(
    ...
    collectKeyboardMatches: function(keyboard) {
        ...
        for (var i = 0; i < this.password.length - 1; i++) {
            if (keyboard.areAdjacent(this.password[i], this.password[i + 1])) {
                ...
                matches[matches.length] = {
                    pattern: currentPath,
                    entropy: this.calculateKeyboardEntropy(
                        currentPath,
                        currentTurns,
                        keyboard
                    ),
    ...
    collectRepetitionMatches: function() {
        ...
        var singleMatches =
            this.password.match(this.regex["repetition"]["single"]) || [];
        ...
        var groupMatches =
            this.password.match(this.regex["repetition"]["group"]) || [];
    ...
    collectSequenceMatches: function() {
        ...
        for (var i = 0; i < this.password.length; i++) {
            ...
            for (var j = i + 2; j <= this.password.length; j++) {
                ...
                if (
                    this.sequences["lower"].indexOf(string) >= 0 &&
                    string.length > lowerSeq.length
                ) {
                    lowerSeq = original;
                }
                if (
                    this.sequences["lower"].indexOf(reversed) >= 0 &&
                    string.length > lowerRevSeq.length
                ) {
                ...
                if (
                    this.sequences["numbers"].indexOf(string) >= 0 &&
                    string.length > numberSeq.length
                ) {
                    numberSeq = original;
                }
                if (
                    this.sequences["numbers"].indexOf(reversed) >= 0 &&
                    string.length > numberSeq.length
                ) {
